...
- v2.14.0 = HL-MEGA-003 (PTH: HM05): Corcovado session prep doc + riff library verification (deployed 2026-03-14). (1) HL-048: Jazz riff library verified — 10 riffs, GET endpoints only (/api/v1/riffs/, /api/v1/riffs/{id}/play, /api/v1/riffs/{id}/transpose/{semitones}). Frontend riffs.html with card grid, key/tag filtering. No POST endpoint needed (static seed data). (2) Corcovado analysis: `docs/Corcovado_Darren_Review_March_2026.md` created — full measure-by-measure chord analysis for Darren session 2026-03-18. Key=A major, 113 BPM, 2/2 time, 48 measures. D7 m21 labeled IV7 (secondary dominant question). Fm9 m41 labeled vim9 correctly (Fm9 formula [0,3,7,10,14] confirmed in CHORD_TEMPLATES).

- v2.15.0 = HL-MEGA-004 (PTH: HM06): Full playback stack rework + Darren prep (deployed 2026-03-15). (1) HL-035: Full score playback rework — 3-button transport (Chords/Score/Full Song) replaces checkbox toggles. Score mode fetches /notes endpoint, schedules via Tone.Part, duration from BPM, respects currentTranspose. (2) HL-036: Chord playback — Chords mode arpeggiated at octave 3, 80ms delay between notes, uses HarmonyAudio.parseChord(). (3) HL-050: Full Song mode — melody (scorePart, normal octave) + chords (part, octave 2) on same Tone.Transport. Single stop kills both. (4) HL-006: Secondary dominant detection — `_detect_secondary_dominants()` in analysis_service.py flags all dom7 quality chords as `secondary_dominant_candidate=True`. P5-resolving dom7 chords also get `secondary_dominant_target`. `_enrich_secondary_dominants()` in analysis.py applies same logic to cached results. Chord modal shows "Interpret as: IV7 | V7/V" toggle for secondary dominant candidates. Corcovado song_id=87 (308 notes, 113 BPM, 2/2, A major). HC01 BV-01: PENDING PL audio browser verification.

### What's Next (updated 2026-03-15)
