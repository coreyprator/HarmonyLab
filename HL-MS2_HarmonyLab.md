# CC Task: HL-MS2 — HarmonyLab Fixes + MIDI Features
...
Fix 3 remaining UAT failures from v2.0.1, then add MIDI-based features that PL identified as high-value during testing. The MIDI real-time chord identification was a breakthrough ("Wow! That's amazing!" — PL during UAT). This sprint builds on that momentum.

**Part A — Fix 3 failures (unblocks PL auditing):**
1. Song 65 "No chords found" — bug persists despite CC claiming it was fixed
2. Note extraction `[object Object]` error — API returns object, frontend expects string
3. Import Notes same error — same root cause

**Part B — 6 new features (extends MIDI capabilities):**
1. MIDI Quiz Mode — reverse flow: app plays chord, PL identifies on keyboard
2. Show MIDI notes played — real-time display for auditing analysis
3. MIDI chord ID for altered extensions — fix b9/b13/#11 vocabulary gap
...
2. If chords exist in DB, the bug is in the frontend query — the song detail page is fetching with wrong ID or wrong endpoint
...
4. If the API itself fails, fix the backend too — check the voice element detection bug (documented in v2.0.1 SESSION_CLOSEOUT: `.findall('.//voice')` found text elements instead of containers)
5. Test with a .mscz file that has pitched staff notation (not a lead sheet)

**Acceptance:**
- Upload a .mscz file with staff notation on the Notes tab → notes populate in table
- Upload a lead sheet → shows "No note data available" (not an error)
...
### FEATURE 1: MIDI Quiz Mode (P2) — HL-NEW-001
...
**Quiz flow:**
1. App displays a target chord: name + optional Roman numeral (e.g., "Dm7 — ii7 in C")
...
### FEATURE 2: Show MIDI Notes Played (P2) — HL-NEW-002
...
### FEATURE 3: MIDI Chord ID for Altered Extensions (P2) — HL-NEW-003
...
2. Add templates for common altered chords:
   ...
   - 7b13 (1-3-b7-b13)
   - 7#11 (1-3-b7-#11)
   - 7alt (1-3-b7-b9-#9-b13) — match if multiple alterations present
   - 9 (1-3-5-b7-9) — natural 9
   - 13 (1-3-b7-13)
   - maj9 (1-3-5-7-9)
   - m9 (1-b3-5-b7-9)
   - 6/9 (1-3-5-6-9)
3. Handle partial voicings — jazz players often omit the 5th. Don't require all intervals.
4. Handle inversions — the root may not be the lowest note.

**Acceptance:**
- Play G-B-F-Ab → identifies as G7b9 (not just "G")
- Play C-E-Bb-Eb → identifies as C7#9
- Play Dm7 voicing (D-F-A-C) → still works (regression check)

### FEATURE 4: Roman Numeral "?" Fix (P3) — HL-NEW-004
...
### FEATURE 5: Delete Song CRUD (P3) — HL-NEW-005
...
- Add delete icon on song list (optional — may be too dangerous for list view)
...
```
Phase 1: Fix Song 65 (investigate + fix)           → deploy, verify
Phase 2: Fix note extraction errors (FE + BE)      → deploy, verify
Phase 3: Show MIDI notes played (Feature 2)        → deploy, verify
Phase 4: Altered chord templates (Feature 3)        → deploy, verify
Phase 5: MIDI Quiz Mode (Feature 1)                → deploy, verify
Phase 6: Roman numeral fix (Feature 4)             → deploy, verify
Phase 7: Delete song (Feature 5)                   → deploy, verify
...
| 2 | Note extraction works | Upload .mscz → notes populate. Lead sheet → "No note data." Errors show readable messages. |
| 3 | MIDI Quiz Mode | Show chord → PL plays → correct/incorrect + score tracking |
...
| 9 | Version bump | 2.0.1 → 2.1.0 (minor: new features) |
...
- **Deploy to Cloud Run after each phase** — do not batch all changes into one deploy
- **Test against production URL** — no local validation or virtual environments
...
- **Default branch**: main
- **Cloud SQL**: DB_HOST_REDACTED, instance flashcards-db, DB LanguageLearning
...
- **Read INTENT.md and PROJECT_KNOWLEDGE.md first** — report checkpoint codes (Bootstrap v1.4.3 gate)
- **Use mandatory closeout format** — Handoff ID, UAT ID, handoff link, artifact commit hashes
- **Present implementation plan before coding** — wait for PL approval before executing

---

## PRIORITY ORDER (if time-limited)

1. **Song 65 fix** (1 hr — unblocks PL trust in chord analysis)
2. **Note extraction fix** (1 hr — unblocks PL auditing)
3. **MIDI notes display** (1.5 hrs — helps PL test everything below)
4. **Altered chord templates** (2 hrs — core analysis improvement)
5. **MIDI Quiz Mode** (3 hrs — biggest new feature)
