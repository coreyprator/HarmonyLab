...
export function TheoryChat({ open, song, selectedChordIds, onClose, onLogOverride }) {
  ...
  const send = async () => {
    ...
    setMessages((m) => [...m, { who: "you", text: question, meta: selectedChordIds.size > 0 ? `${selectedChordIds.size} chord${selectedChordIds.size > 1 ? "s" : ""} selected` : null }]);
    setDraft("");
    setThinking(true);
    try {
      const result = await api.fetcher("/api/v1/analysis/theory-chat", {
        method: "POST",
        body: JSON.stringify({ song_id: song.id, question, selected_measures: [] }),
      });
      const responseText = result.response || result.answer || result.text || JSON.stringify(result);
      setMessages((m) => [...m, { who: "ai", text: responseText, conf: result.confidence }]);
    } catch (e) {
      setMessages((m) => [...m, { who: "ai", text: `Error: ${e.message}`, conf: 0 }]);
    } finally {
      setThinking(false);
    }
  };
...
export function SongDetail({ song: initialSong, route, onNavigate, toast, prefs }) {
  ...
  // when the underlying song changes, refresh
  useEffectS(() => { setSong(initialSong); }, [initialSong.id, initialSong.title]);
  ...
  const reanalyze = async () => {
    ...
    try {
      await api.fetcher(`/api/v1/analysis/songs/${song.id}`, {
        method: "POST",
        body: JSON.stringify({ key_override: song.manualKeyOverride || null }),
      });
      toast("Re-analysis complete", { meta: `POST /analysis/songs/${song.id} \u00b7 200` });
    } catch (e) {
      toast(`Re-analyze failed: ${e.message}`, { level: "error" });
    } finally {
      setReanalyzing(false);
    }
  };
  ...
  const onExport = (kind) => {
    setExportOpen(false);
    if (kind === "musicxml") { window.location.href = `/api/v1/exports/musicxml/${song.id}`; return; }
    if (kind === "mscz") { window.location.href = `/api/v1/exports/musescore/${song.id}`; return; }
